...
class RefMSpec extends BaseCrossPlatformSpec {
  ...
  def e4 =
    ...
    } yield value).flip.mapError(new Exception(_)).map(_ must beTheSameAs(fail))
  ...
  def e7 =
    ...
    } yield value2).flip.mapError(_ => new Exception("unexpected")).map(_ must beTheSameAs(fail))
  ...
  def e12 =
    (for {
      ...
      .mapError(_ => new Exception("unexpected"))
  ...
  def e13 =
    ...
    } yield value).flip.mapError(e => new Exception(e.toString)).map(_ must beTheSameAs(fail))
}
